topic "SliderCtrlX";
[ $$0,0#00000000000000000000000000000000:Default]
[0 $$1,0#96390100711032703541132217272105:end]
[i448;a25;kKO9;2 $$2,0#37138531426314131252341829483380:class]
[b42;2 $$3,3#13035079074754324216151401829390:normal]
[H6;0 $$4,0#05600065144404261032431302351956:begin]
[i448;a25;kKO9;2 $$5,0#37138531426314131252341829483370:item]
[l288;2 $$6,6#27521748481378242620020725143825:desc]
[l288;i1121;b17;O9;~~~.1408;2 $$7,0#10431211400427159095818037425705:param]
[{_}%EN-US 
[ {{10000@(113.42.0) [s0; [*@7;4 SliderCtrlX]]}}&]
[s1; &]
[s2;:StarIndicator`:`:class: [@(0.0.255)3 classe][3 _][*3 SliderCtrlX][3 _:_][@(0.0.255)3 public
][3 _][*@3;3 Ctrl]&]
[s0;i448;a25;kKO9;@(0.0.255) &]
[s3; [/ Derivé de ][/^topic`:`/`/CtrlCore`/src`/Ctrl`$en`-us`#Ctrl`:`:class^ Ctrl][/ . 
Partie de la bibliothèque ][/^topic`:`/`/Controls4U`/src`/Controls4U`$en`-us^ Cont
rol4U][/ .]&]
[s3; &]
[s3; SliderCtrlX est un widget qui permet d`'entrer une valeur entière 
en faisant glisser un curseur. Il a de nombreuses possibilités 
de plus que [^topic`:`/`/CtrlLib`/src`/Slider`$en`-us`#SliderCtrl`:`:class^ SliderC
trl] :&]
[s3;i150;O0; Echelle ajustable avec graduations majeures et mineures 
ajustables,&]
[s3;i150;O0; La valeur peut être positionnée de manière continue 
ou pas à pas avec une valeur de pas ajustable.&]
[s3;i150;O0; Peut être utilisé comme indicateur, avec curseur non 
déplaçable,&]
[s3;i150;O0; Barre indicatrice de valeur dont la largeur et la couleur 
sont ajustables,&]
[s3;i150;O0; Plusieurs formes de curseur, possibilité d`'avoir un 
curseur personnalisé,&]
[s3;i150;O0; L`'échelle peut être inversée,&]
[s3;i150;O0; Possibilité de positionner directement le curseur en 
un click,&]
[s3;i150;O0; Multiples unités pour l`'échelle, comme les heures/mn/s 
permettant de l`'utiliser comme compteur de temps pour une vidéo.&]
[s3; &]
[s3;%FR-FR Affichage dans le [/ layout designer] :&]
[s3; -|
@@image:3090&324
()
&]
[s3;%FR-FR Affichage dans le programme :&]
[s3; -|
@@image:3082&2338
()
&]
[s3; &]
[ {{10000F(128)G(128)@1 [s0;%FR-FR [*2 Détails du constructeur]]}}&]
[s4; &]
[s5;:SliderCtrlX`:`:SliderCtrlX`(`): [* SliderCtrlX()]&]
[s6; Initialise SliderCtrlX avec le comportement par défaut :&]
[s6;i150;O0; m`_nMin (valeur minimum) `= 0,&]
[s6;i150;O0; m`_nMax (valeur maximum) `= 100,&]
[s6;i150;O0; m`_bInverted (échelle inversée) `= false,&]
[s6;i150;O0; m`_nStep (pas) `= 1,&]
[s6;i150;O0; m`_bRound`_step (pas à pas) `= false,&]
[s6;i150;O0; m`_bJump (saute à la valeur cliquée) `= false,&]
[s6;i150;O0; m`_bUseCustomThumbs (index du curseur personnalisé 
à utiliser) `= 0,&]
[s6;i150;O0; m`_nMajorTicks (pas des graduations majeures) `= 10,&]
[s6;i150;O0; m`_nMinorTicks (pas des graduations mineures) `= 2,&]
[s6;i150;O0; m`_nMajorTickSize (taille des graduations majeures) 
`= 30,&]
[s6;i150;O0; m`_nMinorTickSize (taille des graduations mineures) 
`= 20,&]
[s6;i150;O0; m`_nTickPosition (position des graduations) `= TOP,&]
[s6;i150;O0; m`_nThickness (épaisseur de la barre de valeur) `= 
2,&]
[s6;i150;O0; m`_nSliderType (type de contrôle) `= 0,&]
[s6;i150;O0; m`_nThumbNumber (index du curseur prédéfini à utiliser) 
`= 1.&]
[s1; &]
[ {{10000F(128)G(128)@1 [s0; [* Public Method List]]}}&]
[s4;%- &]
[s5;:SliderCtrlX`:`:AddOutCtrl`(Ctrl`*`):%- [_^SliderCtrlX^ SliderCtrlX][@(0.0.255) `&]_[* A
ddOutCtrl](_[_^Ctrl^ Ctrl][@(0.0.255) `*]_[*@3 c]_)&]
[s0;l288;%- Ajoute un Ctrl extérieur en tant que dernier.&]
[s7;i1120;a17; [%-*@3 c]-|Pointeur sur le Ctrl enfant à la fin de la 
liste des enfants. Notez que U`+`+ ne prend jamais possession 
des Ctrls `- n`'appelle jamais delete pour un enfant. Cela permet 
d`'utiliser des Ctrls qui ne sont pas basés sur le [/ heap].&]
[s0; &]
[s4;%- &]
[s5;:SliderCtrlX`:`:Dec`(`):%- [@(0.0.255) void]_[* Dec]()&]
[s6; Decremente la valeur du curseur.&]
[s1;%- &]
[s4;%- &]
[s5;:SliderCtrlX`:`:GetData`(`)const:%- [@(0.0.255) virtual] [_^Value^ Value]_[* GetData]()
_[@(0.0.255) const]&]
[s6; Lecture par défaut de la valeur du curseur.&]
[s1;%- &]
[s4;%- &]
[s5;:SliderCtrlX`:`:GetData`(int`)const:%- [@(0.0.255) virtual] [_^Value^ Value]_[* GetData
](_[@(0.0.255) int]_[*@3 nIndex]_)_[@(0.0.255) const]&]
[s6; Lecture de la valeur pré`-positionnée à l`'index donné.&]
[s7;i1120;a17; [%-*@3 nIndex]-|Index de la valeur pré`-positionnée 
à renvoyer.&]
[s1; &]
[s4;%- &]
[s5;:SliderCtrlX`:`:GotFocus`(`):%- [@(0.0.255) virtual] [@(0.0.255) void]_[* GotFocus]()&]
[s6; Cette méthode est appelée quand Ctrl reçoit le focus. L`'implémentation 
par défaut est de rafraîchir l`'affichage.&]
[s1;%- &]
[s4;%- &]
[s5;:SliderCtrlX`:`:Inc`(`):%- [@(0.0.255) void]_[* Inc]()&]
[s6; Incremente la valeur du curseur.&]
[s1;%- &]
[s4;%- &]
[s5;:SliderCtrlX`:`:IsVert`(`)const:%- [@(0.0.255) bool]_[* IsVert]()_[@(0.0.255) const]&]
[s6; Teste si le curseur est orienté verticalement.&]
[s1;%- &]
[s4;%- &]
[s5;:SliderCtrlX`:`:Key`(dword`,int`):%- [@(0.0.255) virtual] [@(0.0.255) bool]_[* Key]([_^dword^ d
word]_[*@3 key], [@(0.0.255) int]_[*@3 repcnt])&]
[s6; Cette méthode gère l`'entrée clavier. Quand un évènement 
clavier intervient et que l`'un des Ctrls de l`'application a 
le focus, sa méthode [* Key] est appelée. Si la méthode Key 
renvoie vrai, le traitement de l`'événement est terminé. S`'il 
renvoie faux, U`+`+ appelle la méthode Key du contrôle parent 
et recommence jusqu`'à ce que soit vrai est renvoyé, soit le 
contrôle n`'a pas de parent. Si un tel contrôle de niveau supérieur 
est atteint, U`+`+ appelle sa méthode HotKey. L`'implémentation 
par défaut est de retourner immédiatement s`'il s`'agit d`'un 
INDICATEUR, et de traiter les touches de flèches en appelant 
Dec() pour K`_LEFT ou K`_DOWN ou Inc() pour K`_RIGHT ou K`_UP.&]
[s7;i1120;a17; [%-*@3 key]-|La touche qui a été pressée.&]
[s7;i1120;a17; [%-*@3 repcnt]-|compteur de répétition.&]
[s1; &]
[s4;%- &]
[s5;:SliderCtrlX`:`:LeftDown`(Point`,dword`):%- [@(0.0.255) virtual] 
[@(0.0.255) void]_[* LeftDown]([_^Point^ Point]_[*@3 pos], [_^dword^ dword]_[*@3 keyflags])&]
[s6; Traite le bouton gauche de la souris si le curseur est un CONTROLE. 
Retourne immédiatement si c`'est un INDICATEUR.&]
[s7;i1120;a17; [%-*@3 pos]-|Position du pointeur de souris en coordonnées 
d`'affichage.&]
[s7;i1120;a17; [%-*@3 keyflags]-|Combinaison de drapeaux de touches 
pour les touches Shift, Ctrl et Alt.&]
[s1; &]
[s4;%- &]
[s5;:SliderCtrlX`:`:LeftRepeat`(Point`,dword`):%- [@(0.0.255) virtual] 
[@(0.0.255) void]_[* LeftRepeat]([_^Point^ Point]_[*@3 p], [_^dword^ dword]_[*@3 f])&]
[s6;%- Cette méthode est appelée répétitivement par l`'implémentation 
par défaut de MouseEvent quand le bouton gauche de la souris 
est pressé un certain temps, imitant l`'auto`-répétition du 
clavier.&]
[s7;i1120;a17; [%-*@3 p]-|Position du pointeur de souris en coordonnées 
d`'affichage.&]
[s7;i1120;a17; [%-*@3 f]-|Combinaison de drapeaux de touches pour les 
touches Shift, Ctrl et Alt.&]
[s1; &]
[s4;%- &]
[s5;:SliderCtrlX`:`:LeftUp`(Point`,dword`):%- [@(0.0.255) virtual] 
[@(0.0.255) void]_[* LeftUp]([_^Point^ Point]_[*@3 pos], [_^dword^ dword]_[*@3 keyflags])&]
[s6; Traite le bouton droit de la souris si le curseur est un CONTROLE. 
Retourne immédiatement si c`'est un INDICATEUR.&]
[s7;i1120;a17; [%-*@3 pos]-|Position du pointeur de souris en coordonnées 
d`'affichage.&]
[s7;i1120;a17; [%-*@3 keyflags]-|Combinaison de drapeaux de touches 
pour les touches &]
[s1; &]
[s4;%- &]
[s5;:SliderCtrlX`:`:LostFocus`(`):%- [@(0.0.255) virtual] [@(0.0.255) void]_[* LostFocus]()
&]
[s6; Cette méthode est appelée quand le contrôle perd le focus. 
L`'implémentation par défaut est de rafraïchir l`'affichage.&]
[s1;%- &]
[s4;%- &]
[s5;:SliderCtrlX`:`:MinMax`(int`,int`):%- [_^SliderCtrlX^ SliderCtrlX][@(0.0.255) `&]_[* Mi
nMax]([@(0.0.255) int]_[*@3 `_min], [@(0.0.255) int]_[*@3 `_max])&]
[s6; Restreint le curseur aux valeurs `_min et `_max.&]
[s7;i1120;a17; [%-*@3 `_min]-|Valeur minimum.&]
[s7;i1120;a17; [%-*@3 `_max]-|Valeur maximum.&]
[s1; &]
[s4;%- &]
[s5;:SliderCtrlX`:`:MouseMove`(Point`,dword`):%- [@(0.0.255) virtual] 
[@(0.0.255) void]_[* MouseMove]([_^Point^ Point]_[*@3 pos], [_^dword^ dword]_[*@3 keyflags])&]
[s6;%- [%% Cette méthode est appelée ]par l`'implémentation par 
défaut de MouseEvent quand le pointeur de la souris survole 
le contrôle.&]
[s7;i1120;a17; [%-*@3 pos]-|Position du pointeur de souris en coordonnées 
d`'affichage.&]
[s7;i1120;a17; [%-*@3 keyflags]-|Combinaison de drapeaux de touches 
pour les touches &]
[s1; &]
[s4;%- &]
[s5;:SliderCtrlX`:`:Paint`(Draw`&`):%- [@(0.0.255) virtual] [@(0.0.255) void]_[* Paint]([_^Draw^ D
raw][@(0.0.255) `&]_[*@3 w])&]
[s6;b17;a17;%FR-FR Cette méthode est appelée quand la partie visible 
de SliderCtrlX doit être redessinée.&]
[s7;i1120;a17;%FR-FR [*C@3 w]-|Cible des opération de dessin.&]
[s1; &]
[s4;%- &]
[s5;:SliderCtrlX`:`:SetData`(const Value`&`):%- [@(0.0.255) virtual] 
[@(0.0.255) void]_[* SetData]([@(0.0.255) const]_[_^Value^ Value][@(0.0.255) `&]_[*@3 v])&]
[s6; Positionne le curseur dans sa gamme de valeurs.&]
[s7;i1120;a17; [%-*@3 v]-|Nouvelle valeur.&]
[s6;*@3%- &]
[s4;%- &]
[s5;:SliderCtrlX`:`:SetThumbType`(int`):%- [_^SliderCtrlX^ SliderCtrlX][@(0.0.255) `&]_[* S
etThumbType]([@(0.0.255) int]_[*@3 n][@(0.0.255) `=][@3 0])&]
[s6; Sélectionne l`'index en fonction du type donné&]
[s7;i1120;a17; [%-*@3 n]-|Type d`'index choisi.&]
[s0; -|0 : 
@@image:81&75
(BA0ADAAAAAAAAAAAAHiclY1BCgAwCMP8/7v8VwcDD3O2VKEnExJBD/xF+ZrtZObGQXMc7/I1wwFxlPfwRutrGK2RFy3aEC3JDy1sdwC727HZ)
   1 : 
@@image:137&75
(BBYADAAAAAAAAAAAAHicY2BgYEhLS/tPTcwANTMyMvJ/T08PVTDILJj5MLEjR45gxUDr//+vrgZjEBuXOpg5yObiUkuKuTCzR80d+ubSIp1RI19MnDgRI19QA5eUlKDkYwBFHsSp)
   2 : 
@@image:68&156
(BAsAGQAAAAAAAAAAAHicY2BgYDA2Nv5PCi4qKsKJsak7cuQIGP9nYICz0dUjq0NXC1M/qnZU7aha/GqJzW/E5mMGIssHkDoAEcm1lw==)
   3 : 
@@image:68&187
(BAsAHgAAAAAAAAAAAHic7dPBCcAwCAVQd+lEWaTTdYHQUxbqNeEfBPsR9RqIIEZ4aHKIyD/uMb6rtQcpScCiZhZuikycUSNftXpPtp6vWvt2z1pftbYPbe9v1drdxx67o/X+Bqe6zMJZG3l27HUWuwVWkLsP)
   4 : 
@@image:68&187
(BAsAHgAAAAAAAAAAAHic7dJBCgAhCAVQ738u7+VAYGjZ97dpNYIbe0SpImVYXd6dqhrhh/MEPhoBPjmPwtuS6Q50Ft/Q/O+3DyyxOzc2ucaztnQHz1roFv/Mnmqg/9CiXkbbzdMtcNOTbvoPMCCqHw==)
&]
[s1;*@3%- &]
[s4;%- &]
[s5;:SliderCtrlX`:`:SetValue`(const Value`&`,int`):%- [_^Value^ Value]_[* SetValue](_[@(0.0.255) c
onst]_[_^Value^ Value][@(0.0.255) `&]_[*@3 v], [@(0.0.255) int]_[*@3 nIndex]_`=_[@3 0]_)&]
[s6; Prépositionne le curseur dans sa gamme de valeurs.&]
[s7;i1120;a17; [%-*@3 v]-|Nouvelle valeur.&]
[s7;i1120;a17; [%-*@3 nIndex]-|Index du Vecteur de pré`-positionnement.&]
[s1;*@3%- &]
[s4;%- &]
[s0;%- &]
[ {{10000F(128)G(128)@1 [s0; [* Protected Method List]]}}&]
[s5;@(0.0.255)%- &]
[s5;:SliderCtrlX`:`:DrawTick`(Draw`&`,MAJORMINOR`,HOVE`,int`,int`):%- [@(0.0.255) void]_
[* DrawTick](_[_^Draw^ Draw]_`&[*@3 w], MAJORMINOR_[*@3 Type], HOVE_[*@3 Orientation], 
[@(0.0.255) int]_[*@3 nPos], [@(0.0.255) int]_[*@3 nVal]_)&]
[s6; Dessine une graduation à nPos sur le canvas&]
[s7;i1120;a17;%FR-FR [%-*@3 w][%% -|]Cible des opération de dessin.&]
[s7;i1120;a17; [%-*@3 Type]-|Indique si la graduation est majeure ou 
mineure.&]
[s7;i1120;a17; [%-*@3 Orientation]-|Indique si le curseur est horizontal 
ou vertical.&]
[s7;i1120;a17; [%-*@3 nPos]-|Indique la position de la graduation.&]
[s7;i1120;a17; [%-*@3 nVal]-|Indique la valeur à afficher.&]
[s1; &]
[ {{10000F(128)G(128)@1 [s0; [* Private Method List]]}}&]
[s5;@(0.0.255)%- &]
[s5;:SliderCtrlX`:`:ClientToSlider`(int`)const:%- [@(0.0.255) int]_[* ClientToSlider]([@(0.0.255) i
nt]_[*@3 p])_[@(0.0.255) const]&]
[s0;l288;%- [2 Convertit les coordonnées sur le canevas en valeurs 
du curseur.]&]
[s7;i1120;a17; [%-*@3 p]-|Coordonnées à convertir.&]
[s1; &]
[s4;%- &]
[s5;:SliderCtrlX`:`:HoVe`(int`,int`)const:%- [@(0.0.255) int]_[* HoVe]([@(0.0.255) int]_[*@3 x
], [@(0.0.255) int]_[*@3 y])_[@(0.0.255) const]&]
[s6; Renvoie [*@3 x] pour un curseur horizontal ou [*@3 y] pour un curseur 
vertical.&]
[s7;i1120;a17; [%-*@3 x]-|Valeur à retourner pour un curseur horizontal.&]
[s7;i1120;a17; [%-*@3 y]-|Valeur à retourner pour un curseur vertical.&]
[s1; &]
[s4;%- &]
[s5;:SliderCtrlX`:`:HoVeR`(int`&`,int`&`)const:%- [@(0.0.255) int`&]_[* HoVeR]([@(0.0.255) i
nt`&]_[*@3 x], [@(0.0.255) int`&]_[*@3 y])_[@(0.0.255) const]&]
[s6; Renvoie [@3 `&][*@3 x] pour un curseur horizontal ou [@3 `&][*@3 y] 
pour un curseur vertical.&]
[s7;i1120;a17; [%-*@3 x]-|Reference à retourner pour un curseur horizontal.&]
[s7;i1120;a17; [%-*@3 y]-|Reference à retourner pour un curseur vertical.&]
[s1; &]
[s4;%- &]
[s5;:SliderCtrlX`:`:SliderToClient`(int`)const:%- [@(0.0.255) int]_[* SliderToClient]([@(0.0.255) i
nt]_[*@3 v])_[@(0.0.255) const]&]
[s6;%- Convertit les valeurs du curseur en coordonnées sur le canevas.&]
[s7;i1120;a17; [%-*@3 v]-|Valeur à convertir.&]
[s1; &]
[s0;%FR-FR ]]