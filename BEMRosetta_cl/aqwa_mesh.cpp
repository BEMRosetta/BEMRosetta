...
void AQWAMesh::SaveDat(String fileName, const UArray<Mesh*> &meshes, const UArray<Surface> &surf, double rho, double g, bool y0z, bool x0z) {
	...
	out << SaveDat(meshes, surf, rho, g, y0z, x0z);
}

String AQWAMesh::SaveDat(const UArray<Mesh*> &meshes, const UArray<Surface> &surfs, double rho, double g, bool y0z, bool x0z) {
	...
	VectorXd head = VectorXd::LinSpaced(9, -180, 180);
	...
	<< "TITLE               " << "\n"
	<< "NUM_CORES         4	" << "\n"
	<< Format("OPTIONS %s%s%s", getQTF ? "AQTF " : "", "GOON", getQTF ? "AQTF " : "") << "\n"
	<< "OPTIONS AHD1 " << "\n"
	...
	for (int ib = 0; ib < meshes.size(); ++ib)
		ret << Format("    %2d         %5d  %d", ib+1, 9800+ib, FDS(meshes[ib]->GetMass(), 8, true));
	...
	for (int ib = 0; ib < meshes.size(); ++ib) {
		...
		for (int ih = 0; ih < head.size(); ++ih) 
			ret << Format("    %2d%s   %2d   %2d %s\n", ib+1, "DIRN", ih+1, ih+1, FDS(hrtz[ih], 9, true));	
		ret	<< " END\n";
		ret << "********************************************************************************\n";
	}
/*	ret
