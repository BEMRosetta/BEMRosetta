...
bool Wamit::Load(String file, Function <bool(String, int)> Status) {
	...
	try {
		...
		} else if (S(".1.2.3.3sc.3fk.hst.4.12d.12s.cfg.frc.pot").Find(ext) >= 0) {
...
void Wamit::ProcessFirstColumn1_3(UVector<double> &w, UVector<double> &T) {	
	if (IsNull(iperout)) {
		if (w.size() < 2)
			return;
		if (w[0] > w[1]) {
