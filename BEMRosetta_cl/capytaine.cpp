...
String CapyNC_Load(const char *file, UArray<Hydro> &hydros, int &num) {
	...
	try {
		...
		MatrixXd cb;
		if (cdf.ExistVar("center_of_buoyancy")) {
			cdf.GetDouble("center_of_buoyancy", cb);
			cb.transposeInPlace();
			if (!(cb.rows() == 3 && cb.cols() == Nb))
				throw Exc(t_("Wrong dimension in cb"));
		} else 
			cb = MatrixXd::Zero(3, Nb);
		...
		for (int irho = 0; irho < _rho.size(); ++irho) {
			for (int ih = 0; ih < _h.size(); ++ih) {
				...
				if (!hy.dt.sc.IsEmpty() && !IsNum(hy.dt.sc))
					throw Exc(t_("Nans found in scattering force"));
				if (!hy.dt.fk.IsEmpty() && !IsNum(hy.dt.fk))
					throw Exc(t_("Nans found in Froude Krylov force"));
				if (!hy.dt.rao.IsEmpty() && !IsNum(hy.dt.rao))
					throw Exc(t_("Nans found in RAO"));
...
void Nemoh::SaveCase_Capy(String folder, int numThreads, bool withPotentials, bool withMesh, bool x0z, bool y0z, const UArray<Body> &lids) const {
	...
	if (!IsNull(numThreads) && numThreads > 0) 
		spy << "os.environ[\"OMP_NUM_THREADS\"] = \"" << numThreads << "\"\n"
