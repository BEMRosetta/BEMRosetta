...
bool Nemoh::Load_Inf(String fileName) {
	if (hd().Nb != 1)
		throw Exc(Format("SeaFEM_Nemoh only allows one body, found %d", hd().Nb));
		
	hd().cg.setConstant(3, 1, nan(""));
	hd().cb.setConstant(3, 1, nan(""));
	hd().Vo.SetCount(1, nan(""));
	hd().C.SetCount(1);
	hd().C[0].setConstant(6, 6, nan(""));   
...
bool Nemoh::Load_Radiation(String fileName) {
	...
	for (int k = 0; k < hd().Nf; ++k) {
		hd().A[k].setConstant(hd().Nb*6, hd().Nb*6, nan(""));
		hd().B[k].setConstant(hd().Nb*6, hd().Nb*6, nan(""));
	}
	int ibodydof = 0;
...
bool Nemoh::LoadMesh(String fileName) {
	...
	try {
		...
		if (f.GetInt(0) != 2)
			throw Exc("Format error in Nemoh .dat mesh file");
		if (f.GetInt(1) == 1)
			mh().x0z = true;
		...
		if (mh().Check())
			throw Exc("Wrong nodes found in Nemoh .dat mesh file");
		mh().GetLimits();
	} catch (Exc e) {
		hd().PrintError("\nError: " + e);
		mh().lastError = e;
