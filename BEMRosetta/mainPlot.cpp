...
bool MainPlot::Load(const UArray<Hydro> &hydro, const MainBEM &mbm, const UVector<int> &ids) {
	...
	ABFZ_source_p2.SetCount(ids.size());
	ABFZ_source2_p2.SetCount(ids.size());
...
bool MainPlot::Load(const Hydro &hy, const MainBEM &mainBem) {
	...
	ABFZ_source_p2.SetCount(1);
	ABFZ_source2_p2.SetCount(1);
...
void MainPlot::LoadEach(const Hydro &hy, int id, bool &loaded, int idc) {
	...
	} else if (dataToShow == Hydro::DATA_STS2) {
...
void MainPlot::Clear() {
	...
	ABFZ_source_p2.Clear();
	ABFZ_source2_p2.Clear();
	Ainf_source.Clear();
	A0_source.Clear();
	B_H_source.Clear();
	A_P_source.Clear();
