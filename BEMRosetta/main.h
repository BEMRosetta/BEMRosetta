...
public:
	...
	virtual inline double y(int64 id) {
		...
		case Hydro::PLOT_RAO_1:		return show_ma_ph ? abs (data->R_(data->rao, jdf, int(id), idf)) : 
														real(data->R_(data->rao, jdf, int(id), idf));
		case Hydro::PLOT_RAO_2:		return show_ma_ph ? arg (data->R_(data->rao, jdf, int(id), idf)) : 
														imag(data->R_(data->rao, jdf, int(id), idf));
